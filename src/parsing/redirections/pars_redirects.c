...
/* This functions goal is to create the file
and set the fd of the file created in thecommand token. */
...
if expr_index is 0, and we have ls |  rev | cat,
it will return the token with ls.
...
/* The goal of this function is to, when given a tokenlist,
find where the output of the command token be sent to.
Example: ls > file.txt
Our function will detect the ">", create a file named file.txt
and store its file descriptor in the command token.
Likewise for the "<" but it will not create the file,
and store the file descriptor so the command can read from it. */
