...
/* Creates a new token, adds a duplicate of the element
...
If the element is a var ($VAR) or has a variable within,
it will replace the variable by its value.
we replace it by it's value so we can tokenize it.
exemple: echo $USER will give this linked list: echo -> username
As you can see, we return a pointer to line + the lenght of the element
we just converted so we can keep going trough the line. */
